...
assert.equal(await page.locator('dialog .profile-avatar img').count(),1);assert.equal(await page.locator('dialog .fit-frame-particles i').count(),8);
...
await page.locator('[data-tab=editor]').click();await page.locator('[name=name]').fill('Marco verde');await page.locator('[name=effect]').selectOption('candy');await page.locator('#style-try').click();assert.equal(await page.locator('dialog .fit-frame-particles i').count(),8);await page.locator('[data-close]').click();
