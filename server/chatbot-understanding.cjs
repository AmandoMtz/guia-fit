...
function understand(message, context = {}, options = {}) {
  ...
  if (/\b(chat|mensaje|mensajes|conversacion)\b/.test(q) && /vendedor|pedido|imagen|foto|dura|temporal/.test(q)) return { answer:reply('Abre tu pedido en “Comidas” para conversar con el vendedor y enviar imágenes. El chat y sus imágenes duran 12 horas. El pedido conserva su registro. Consulta el tiempo restante dentro de la conversación.') };
