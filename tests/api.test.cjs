...
test("API Render/Aiven: autenticación real, permisos y verificación", async (t) => {
  ...
  await engine.exec(
    fs.readFileSync(path.join(root, "backend/migrations/002_food.sql"), "utf8"),
  );
  await engine.exec(
    fs.readFileSync(
      path.join(root, "backend/migrations/003_profiles.sql"),
      "utf8",
    ),
  );
  await engine.exec(
  ...
  for(const name of fs.readdirSync(path.join(root,'backend/migrations')).sort().filter(n=>n.endsWith('.sql')&&n>'004_events.sql')) await engine.exec(fs.readFileSync(path.join(root,'backend/migrations',name),'utf8'));
