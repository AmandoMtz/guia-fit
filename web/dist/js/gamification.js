(function(root){
 ...
 const previewText=item=>item.slot==='frame'?'FIT':item.slot==='theme'?'Guía FIT':item.slot==='font'?'Aa FIT':'Hola, ¿cómo va tu día?';
 ...
 async function render(c){
  ...
  try{
   ...
   if(tab==='progress'){
    ...
    content.innerHTML=`<div class="reward-settings"><label><input type="checkbox" id="reward-motion" ${data.animations?'checked':''}> Activar animaciones suaves</label><p>Personaliza colores, tipografía, marcos y chats. Puedes cambiar de estilo cuando quieras.</p></div><div class="reward-grid">${data.catalog.map(item=>{const owned=data.inventory.includes(item.id),active=data.equipped[item.slot]===item.id,locked=item.minLevel&&data.level<item.minLevel;return `<article class="reward-card ${locked?'reward-locked':''}"><div class="reward-preview ${esc(item.id)}" aria-hidden="true"><span>${previewText(item)}</span></div><h3>${esc(item.name)}</h3><p>${esc(item.description)}</p><p><b>${locked?'Se desbloquea en nivel '+item.minLevel:owned?'Ya es tuyo':item.price+' monedas'}</b></p><button class="btn ${active?'secondary':''}" data-item="${item.id}" ${locked||(!owned&&data.coins<item.price)?'disabled':''}>${locked?'Nivel '+item.minLevel:active?'Quitar':owned?'Equipar':'Canjear'}</button></article>`;}).join('')}</div>`;
