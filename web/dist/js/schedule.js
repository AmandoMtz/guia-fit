(function (root) {
  ...
  function review(c, draft, imported = false, editId = null) {
    ...
    const redraw = () => {
      ...
      d.querySelector("#review-classes").innerHTML = draft.classes.length
        ? draft.classes
            .map(
              (x) =>
                `<div class="review-row"><div><b>${c.esc(x.subject || "Materia por completar")}</b><span>${S.days[x.day - 1] || "Día por confirmar"} · ${c.esc(x.start)}–${c.esc(x.end)}</span><small>${c.esc(x.teacher || "Maestro pendiente")} · Grupo ${c.esc(x.group || "—")} · ${c.esc(x.classroom || "Salón pendiente")}</small>${S.validate(x) ? '<small class="field-error">Completa los datos de esta clase</small>' : ""}</div><div class="button-row"><button class="btn secondary small" type="button" data-edit-class="${c.esc(x.id)}">Editar</button><button class="text-button danger" type="button" data-remove-class="${c.esc(x.id)}">Quitar</button></div></div>`,
            )
            .join("")
        : '<div class="empty">Agrega la primera clase. Usa una entrada por día y bloque horario.</div>';
      d.querySelectorAll("[data-edit-class]").forEach(
        (b) =>
          (b.onclick = () =>
            editClass(
              c,
              draft,
              draft.classes.find((x) => x.id === b.dataset.editClass),
              redraw,
            )),
      );
      d.querySelectorAll("[data-remove-class]").forEach(
        (b) =>
          (b.onclick = () => {
            draft.classes = draft.classes.filter(
              (x) => x.id !== b.dataset.removeClass,
            );
            redraw();
          }),
      );
    ...
    d.querySelector("#add-class").onclick = () => {
      if (draft.classes.length >= 120) {
        c.toast("Puedes guardar hasta 120 bloques de clase.");
        return;
      }
    ...
    f.onsubmit = async (e) => {
      ...
      try {
        if (!draft.classes.length)
          throw Error("Agrega al menos una clase para generar el calendario.");
        const invalid = draft.classes.find((x) => S.validate(x));
        if (invalid)
          throw Error(
            `Revisa ${invalid.subject || "la clase"}: ${S.validate(invalid)}`,
          );
        if (c.state.user?.id !== draft.userId)
        ...
        d.querySelector("#review-error").textContent = e.message;
      } finally {
        b.disabled = false;
      }
    };
    redraw();
    if (editId) {
      const current = draft.classes.find((x) => x.id === editId);
      if (current) editClass(c, draft, current, redraw);
    }
  }
  function editClass(c, draft, current, done) {
    ...
    const d = c.dialog(
      `<div class="dialog-content"><h2>${current ? "Editar clase" : "Agregar clase"}</h2><form>${[
        ["subject", "Materia", 160],
        ["teacher", "Maestro", 160],
        ["classroom", "Salón", 100],
        ["group", "Grupo", 40],
        ...
        .map(
          ([n, l, max]) =>
            `<label class="field">${l}<input name="${n}" ${n === "group" ? "" : "required"} maxlength="${max}" value="${c.esc(x[n])}"></label>`,
        )
        .join(
          "",
    ...
    d.querySelector("form").onsubmit = (e) => {
      ...
      const next = { ...x, ...data, day: Number(data.day) };
      const err = S.validate(next);
      if (err) {
        d.querySelector("[role=alert]").textContent = err;
        return;
      }
      if (current) draft.classes[draft.classes.indexOf(current)] = next;
      else draft.classes.push(next);
      d.close();
      done();
    };
  }
